...
public class MapNav : MonoBehaviour 
{
    ...
    public float fixLat = 69.69971f;					   			//Latitude
    public float fixLon = 19.00721f;							//Longitude
	...
	IEnumerator MapPosition(){
	   ...
	   //------------------------------------------------------------------	//
	    ...
	    multiplier = 2;
		url= "http://maps.google.com/maps/api/staticmap?center="+fixLat+","+fixLon+"&zoom="+zoom+"&scale=2&size=640x640&format=png&maptype="+maptype[index]+"&sensor=false&key="+key;
		tempLat = fixLat; 
		tempLon = fixLon;
